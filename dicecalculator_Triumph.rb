...
#Triumph and Despair.. good luck, Nik.
...
dice_string.each_char do |die|
  ...
  temp_die_grid.each { |x| x.each { |y| y.each { |z| z.collect! { |n| n / possibilities } } } }
  
  #make a new grid based on the type of die the temp_die is, good or bad.
  result_grid_temporary = /[BAP]/ === die ? \
  Array.new(success_temp) { Array.new(advantage_temp) { Array.new(triumph_temp) { Array.new(1, 0.0) } } } : \
  Array.new(failure_temp) { Array.new(threat_temp)    { Array.new(1) { Array.new(despair_temp, 0.0) } } }
...
result_grid.each { |x| x.each { |y| y.each { |z| z.collect! { |n| n * 100 } } } }
